export default function SectionHeading({ eyebrow, title, text, center = false, light = false }) {
  return (
    <div className={center ? 'mx-auto max-w-3xl text-center' : 'max-w-3xl'}>
      <div className={`mb-4 flex items-center gap-3 text-xs font-extrabold uppercase tracking-[0.2em] ${center ? 'justify-center' : ''} ${light ? 'text-blue-200' : 'text-brand-600'}`}>
        <span className="h-0.5 w-8 rounded-full bg-current" />
        {eyebrow}
        {center && <span className="h-0.5 w-8 rounded-full bg-current" />}
      </div>
      <h2 className={`text-balance text-4xl font-black leading-[1.08] tracking-[-0.04em] md:text-5xl ${light ? 'text-white' : 'text-brand-950'}`}>
        {title}
      </h2>
